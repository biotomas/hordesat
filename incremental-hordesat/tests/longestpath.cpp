		...
		if (c == 'p') {
			...
			if (2 != sscanf(pline, "p edge %d %d", &(g.verticesCount), &(g.edgesCount))) {
				...
				if (2 != sscanf(pline, "p sp %d %d", &(g.verticesCount), &(g.edgesCount))) {
					log(0, "Failed to parse the problem definition line (%s)\n", pline);
				}
			}
			continue;
		}
...
int main(int argc, char **argv) {
	...
	*/
	log(0, "INFO: searching for a longest path from %d to %d\n", start, goal);

	int lb, ub;
	getLowerUpperBound(g, start, goal, lb, ub);
	if (lb > 0) {
		log(0, "INFO: DFS found path of length %d, upper bound is %d\n", lb, ub);
	} else {
		log(0, "INFO: no path between start and goal, %d reachable vertices from start\n", ub);
		return 0;
	}
	...
	while (pathLength < ub) {
		...
		log(0, "INFO: trying to find a path of length %d by SAT\n", pathLength+1);
		int res = ipasir_solve(solver);
		log(0, "solver finished ======================================\n");
		if (res == 10) {
			log(0, " path found!\n");
			best = pathLength+1;
		} else {
			log(0, " path not found!\n");
		}
		pathLength++;
	}
	log(0, "RES: Longest path has %d vertices.\n", best);
	return 0;
}
